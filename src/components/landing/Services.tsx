...
const services = [
  {
    ...
    hook: "Most clients go live in under 7 days.",
  },
  {
    icon: ShoppingBag,
  ...
  <section id="services" className="py-24 md:py-32 bg-white">
    <div className="container">
      ...
      <div className="mt-12 grid gap-5 sm:grid-cols-2">
        {services.map(({ icon: Icon, title, desc, hook }, i) => (
          <article
            key={title}
            className="reveal group bg-white border border-line rounded-lg p-8 tilt-card hover:border-ink/20"
            data-stagger={i}
          >
            <div className="flex items-start justify-between">
              <div className="h-10 w-10 rounded-md border border-line flex items-center justify-center text-ink group-hover:border-ink group-hover:bg-ink group-hover:text-white transition-all duration-300">
                <Icon className="h-5 w-5" strokeWidth={1.6} />
              </div>
              <ArrowUpRight className="h-5 w-5 text-mute group-hover:text-ink group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-all duration-200" />
            </div>
            ...
            <p className="mt-3 font-body text-[13px] font-semibold text-ink/60 italic">{hook}</p>
          </article>
        ))}
      </div>
    </div>
